...
const TVView: React.FC<TVViewProps> = ({ arenaId }) => {
  ...
  useEffect(() => {
    ...
    if (activeMatch?.status === 'playing' && activeMatch?.gameStartTime) {
      ...
      // James: Se não houver data de início (partida zerada), reseta na hora para 0
      setElapsedSeconds(0);
    }
  ...
  return (
    <div className="min-h-screen w-full bg-[#020617] text-white p-8 font-sans overflow-hidden flex flex-col gap-8">
      {/* Modals de Vitória/Vai a 3 sincronizados em tempo real com o Tablet */}
      ...
      {activeMatch?.modals?.victoryData && (
        <VictoryModal 
          ...
          onUndo={()=>{}} 
          isTV={true} 
          arenaColor={activeMatch.modals.victoryData.arenaColor || 'indigo'} 
        />
      )}
