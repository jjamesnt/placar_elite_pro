  ...
  return (
    <div className="w-full p-4 flex flex-col gap-6 animate-in fade-in duration-500 overflow-x-hidden">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        {/* Título Formatado conforme solicitado no print */}
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            {matches.length > 0 && !filterDate && (
              <button 
                ...
                className="px-4 py-2 bg-red-900/20 text-red-500 hover:bg-red-900/40 text-[9px] font-black uppercase tracking-widest rounded-xl transition-colors border border-red-500/20"
              >
                Limpar
              </button>
            )}
      ...
      <div className="space-y-3">
        {filteredMatches.map(match => {
          ...
          return (
            <div 
              key={match.id} 
              className="bg-gray-800/40 border border-gray-700/30 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 hover:border-indigo-500/30 group"
            >
              ...
              <div className="flex-1 flex items-center justify-center gap-4 w-full">
                ...
                <div className="text-gray-700 font-black italic text-[10px]">VS</div>
        ...
        {filteredMatches.length === 0 && (
          <div className="py-20 text-center flex flex-col items-center gap-4 bg-gray-800/20 rounded-[3rem] border border-dashed border-gray-700/30">
            <HistoryIcon className="w-12 h-12 text-gray-700" />
            <p className="text-gray-600 italic uppercase tracking-widest text-[9px]">Histórico vazio.</p>
          </div>
        )}
      </div>

      {matchToDelete && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[110] flex items-center justify-center p-4" onClick={() => setMatchToDelete(null)}>
